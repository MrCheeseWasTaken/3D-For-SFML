...
namespace sf{
    ...
    void Cube::drawInstance(RenderTarget3D& target, const InstanceData* InstanceData, unsigned int instanceCount, const RenderStates3D& states) const {
        
        RenderStates3D stateCopy = states;
        if (Shape3D::hasTexture()) stateCopy.textures.push_back(&Shape3D::getTexture());
        stateCopy.flipTexture = Shape3D::getFlipTexture();

        target.drawInstanced(sf::RenderTarget3D::Cube, InstanceData, instanceCount, stateCopy);
